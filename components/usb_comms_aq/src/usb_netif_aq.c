...
esp_err_t usb_netif_aq_start(void) {
    ESP_LOGI(TAG, "Starting USB ECM network interface...");
    ...
    if (!s_netif_aq) {
        ...
        if (!s_netif_aq) {
            ESP_LOGE(TAG, "Failed to create esp_netif for USB ECM");
            return ESP_FAIL;
        }
    }
    ...
    ESP_LOGI(TAG, "Initializing TinyUSB stack with custom ECM descriptors");
    ...
    ESP_LOGI(TAG, "USB ECM interface started successfully.");
    return ESP_OK;
}
...
void tud_network_link_state_cb(bool up)
{
    if (up) {
        if (!s_network_is_active) {
            ESP_LOGI(TAG, "USB ECM link is UP. Starting network stack.");
        ...
        if (s_network_is_active) {
            ESP_LOGI(TAG, "USB ECM link is DOWN. Stopping network stack.");
...
{
    ESP_LOGI(TAG, "tud_cdc_line_state_cb: itf=%u, dtr=%d, rts=%d", itf, dtr, rts);
    (void)itf;
    (void)rts;
    tud_network_link_state_cb(dtr);
}
